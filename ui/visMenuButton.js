...
class VisMenuButton extends Button
{
	constructor()
	{
		super();
	...
	posUpdate()
	{
		...
		this.buttonOffset = (width * 0.75) - (width * 0.45);
		this.y = height - 45;
	}
	...
	draw()
	{
		...
		if(width > 1000)
		{
			...
			else
			{
				...
				//Draw the menu
				fill(0, 0, 0, 200)
				rect((width * 0.75) - 100, height - 280, 130, 170, 20);

				//Draws out all visualisation names when the menu is open
				for(let [i, e] of vis.visuals.entries())
				{
					textSize(14);
					fill(255);
					text([i + 1] + ") " + e.name, 
